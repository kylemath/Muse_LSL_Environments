...
while os.path.isfile(scriptdir + 'Muse-lsl-Baseline/data/Outside/%s_mac_smith_outside_lsl_baseline.csv'%(partnum)) == True:
    partnum = '00'+str(int(partnum)+1)
    if int(partnum) > 9:
        partnum = '0' + str(int(partnum))

filename    = '%s_mac_smith_outside_lsl_baseline'%(partnum)

default_fname = (scriptdir + 'Muse-lsl-Baseline/data/Outside/%s.csv' % filename)
...
marker_streams = resolve_byprop('name', 'Markers', timeout=30)
...
while os.path.isfile(scriptdir + 'muse-lsl-alexB/Stop_EEG.csv') == True:
#while inlet_marker:
    #check_stop = np.genfromtxt('/home/pi/Experiments/Baseline/Stop_EEG.csv', delimiter=',')
